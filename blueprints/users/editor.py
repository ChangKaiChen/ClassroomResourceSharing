...
async def editor():
    if request.method == 'GET':
        return await render_template('users/editor.html', title='编辑文件', username=current_user.auth_id)
        ...
        try:
            ...
            if file:
                ...
                if file_extension in ['.rar', '.zip', '.7z']:
                    save_path = os.path.join('static', 'uploads', f'{new_article.id}{file_extension}')
